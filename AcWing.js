...
class AcWing {
    ...
    async getContestIndex() {
        ...
        for (let i = 0; i < href.length; ++i) {
            if (href[i] >= '0' && href[i] <= '9') {
                this.contest_index = this.contest_index * 10 + href[i].charCodeAt(0) - '0'.charCodeAt(0);
            }
        }
        Console.log(0, `AcWing竞赛ID为${this.contest_index}的周赛排名开始爬取`, 'BgGreen');
    }
    ...
    writreToEccel(group_data = [], data = []) {
        ...
        workbook.xlsx.writeFile(this.excel_name + '.xlsx')
            .then(function () {
    ...
    async getPage(page = 1) {
        ...
        const { data: html } = await axios({
            ...
            Console.log(0, `AcWing爬取排名第${page}页时出错:${err}`, 'BgRed');
            return;
        });
        const document = new JSDOM(html).window.document;
        let tr_list = document.querySelectorAll('table tr');
        if (!this.excel_name.length) {
            this.excel_name = 'AcWing' + document.querySelector('.activity_title')?.textContent + '排名';
        ...
        Console.log(0, `${this.excel_name}第${page}页已爬取完成`, 'BgGreen');
        return page_rank;
    }
    ...
    async run() {
        try {
            ...
            Console.log(0, `AcWing排名爬取失败:${err}`, 'BgRed');
        }
    }
}
