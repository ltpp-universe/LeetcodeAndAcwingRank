...
class LeetCode {
    ...
    getGroupRank() {
                ...
                join.forEach((tem) => {
                    ...
                    for (let [key] of this.question) {
                        if (this.submission[tem][key]) {
                            tem_data.push(
                                (this.submission[tem][key].fail_count > 0 ? '-' + this.submission[tem][key].fail_count + ' ' : '') +
        ...
        for (let i = 0; i < rank_len; ++i) {
            ...
            for (let [key] of this.question) {
                if (this.submission[i][key]) {
                    tem_data.push(
                        (this.submission[i][key].fail_count > 0 ? '-' + this.submission[i][key].fail_count + ' ' : '') +
        ...
        Console.log(0, `LeetCode第${this.contest_id}场${this.is_double_week ? '双' : ''} 周赛排名爬取完成`, 'BgGreen');
        group_data.sort((a, b) => {
    ...
    writreToEccel(group_data = [], data = []) {
        ...
        workbook.xlsx.writeFile(`LeetCode第${this.contest_id}场${this.is_double_week ? '双' : ''}周赛排名.xlsx`)
            .then(() => {
                Console.log(0, 'Excel文件已保存', 'BgGreen');
            })
            .catch((err) => {
                Console.log(0, '保存Excel文件时出错:' + err, 'BgRed');
            });
    }
    ...
    async getPage(page) {
        while (1) {
            const { data: res } = await axios({
                url: `https://leetcode.cn/contest/api/ranking/${this.is_double_week ? 'biweekly' : 'weekly'}-contest-${this.contest_id}/?pagination=${page++}&region=local`,
                method: 'get',
            }).catch((err) => {
                Console.log(0, `LeetCode第${this.contest_id}场${this.is_double_week ? '双' : ''}周赛排名第${Math.max(0, page - 1)}页爬取出错:${err}`, 'BgRed');
            });
            ...
            Console.log(0, `LeetCode第${this.contest_id}场${this.is_double_week ? '双' : ''}周赛排名第${Math.max(0, page - 1)}页已爬取完成`, 'BgGreen');
        }
    }
    ...
    async getContestTitle() {
        const { data: contest } = await axios({
            ...
            Console.log(0, 'LeetCode周赛记录获取出错:' + err, 'BgRed');
            return;
        });
    ...
    async run() {
        try {
            ...
            Console.log(0, `开始爬取LeetCode第${this.contest_id}场${this.is_double_week ? '双' : ''}周赛排名`, 'BgGreen');
            await this.getPage(1);
            this.getGroupRank();
        } catch (err) {
            Console.log(0, `LeetCode排名爬取失败:${err}`, 'BgRed');
        }
    }
}
